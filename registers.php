...
echo"<center>";
echo"<h3>Registered Username and Email details</h3>";
$sql="SELECT * FROM registry";
$result=mysqli_query($con,$sql) or die(mysqli_error($con));
	if(mysqli_num_rows($result)>0)
		{
		echo"<table class='table' border=1>";
		echo"<tr><th colspan=10>User</th><th colspan=15>Email</th></tr>";
	while($row=mysqli_fetch_assoc($result))
		{
		echo"<tr><td colspan=10>".$row['name1']."<td colspan=15>".$row['email2']."</td></tr>";
		
		}	echo"</table>";
	}
else{
	echo"No reigistration yet";
	}
